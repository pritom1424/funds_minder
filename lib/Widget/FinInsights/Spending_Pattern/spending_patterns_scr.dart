...
import 'package:funds_minder/Widget/FinInsights/Spending_Pattern/spending_pattern_itm.dart';

class SpendingPatternsScr extends StatelessWidget {
  final Map<Filteroptions, List<ChartData>> chartMap;
  final List<Expense> expns;
  const SpendingPatternsScr(this.chartMap, this.expns, {super.key});
            ...
            : Container(
                child: (screenSize.height > 600)
                    ? ListView.builder(
                        ...
                        itemBuilder: (ctx, i) =>
                            (chartMap.values.elementAt(i).isEmpty)
                                ? Center(
                                    child: Padding(
